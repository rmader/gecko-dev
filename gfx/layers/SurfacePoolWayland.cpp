...
NativeSurfaceWayland::~NativeSurfaceWayland() {
  // TODO: Make sure we properly destroy surfaces
  /*GLContextEGL* egl = GLContextEGL::Cast(mGL);
...
RefPtr<NativeSurfaceWayland> SurfacePoolWayland::ObtainSurfaceFromPool(
  ...
  mPoolSize++;
  if (mPoolSize > mPoolSizeLimit) {
    NS_WARNING(nsPrintfCString("Pool size greater than limit (%zu > %zu)",
                               mPoolSize, mPoolSizeLimit)
                   .get());
  }
...
void SurfacePoolWayland::ReturnSurfaceToPool(
  ...
  if (aSurface->mWlSubsurface) {
    wl_subsurface_destroy(aSurface->mWlSubsurface);
    aSurface->mWlSubsurface = nullptr;
  }
}
