...
void NativeLayerRootWayland::SetLayers(
  ...
  for (const RefPtr<NativeLayerWayland>& layer : mSublayers) {
    ...
    Rect surfaceRectClipped =
        Rect(0, 0, layer->mSize.width, layer->mSize.height);
    ...
    Point absPosition = Point(layer->mPosition.x + relPosition.x,
                              layer->mPosition.y + relPosition.y);

    Point scaledSize = layer->mTransform.TransformPoint(
                           Point(layer->mSize.width, layer->mSize.height)) -
                       relPosition;
    float scaleX = scaledSize.x / layer->mSize.width;
    float scaleY = scaledSize.y / layer->mSize.height;
    ...
    int scale = moz_container_wayland_get_scale(mContainer);
    wl_subsurface_set_position(nativeSurface->mWlSubsurface,
                               roundf(surfaceRectClipped.x / scale),
                               roundf(surfaceRectClipped.y / scale));
    wp_viewport_set_destination(nativeSurface->mViewport,
                                roundf(surfaceRectClipped.width / scale),
                                roundf(surfaceRectClipped.height / scale));
...
bool NativeLayerRootWayland::CommitToScreen() {
  ...
  if (!mInitialized) {
    ...
    mViewport = wp_viewporter_get_viewport(viewporter, wlSurface);
    wp_viewport_set_source(mViewport, wl_fixed_from_int(0),
                           wl_fixed_from_int(0), wl_fixed_from_int(1),
                           wl_fixed_from_int(1));
    wp_viewport_set_destination(mViewport, 1, 1);
    moz_container_wayland_surface_unlock(mContainer, &wlSurface);
